...
class TrialSiteTest(unittest.TestCase):
    ...
    def test_verifyIntegrity_head(self) -> None:
        ...
        trial_site = TrialSite(df=pd.DataFrame(columns=['Bestandeseinheit', 'Baumart', 'Baumnummer']), metadata={})
        trial_site.verify_column_integrity(column_scheme)
        self.assertEqual(trial_site.df.dtypes.values.tolist(), [pd.UInt16Dtype(), pd.StringDtype(), pd.UInt32Dtype()])
    ...
    def test_verifyIntegrity_measurements(self) -> None:
        ...
        trial_site = TrialSite(
            df=pd.DataFrame(columns=['D_1980', 'Aus_1980', 'D_2000', 'Aus_2000', 'D_2020', 'Aus_2020']), metadata={})
        trial_site.verify_column_integrity(column_scheme)
        self.assertEqual(trial_site.df.dtypes.values.tolist(), 3 * [pd.Float64Dtype(), pd.UInt16Dtype()])

    def test_verifyIntegrity_combined(self) -> None:
        column_scheme = ColumnScheme(head=self.sample_head_scheme, measurements=self.sample_measurements_scheme)
        trial_site = TrialSite(df=pd.DataFrame(
            columns=['Bestandeseinheit', 'Baumart', 'Baumnummer', 'D_1980', 'Aus_1980', 'D_2000', 'Aus_2000', 'D_2020',
                     'Aus_2020']), metadata={})
        trial_site.verify_column_integrity(column_scheme)
        self.assertEqual(trial_site.df.dtypes.values.tolist(),
                         [pd.UInt16Dtype(), pd.StringDtype(), pd.UInt32Dtype()] + 3 * [pd.Float64Dtype(),
                                                                                       pd.UInt16Dtype()])

    def test_verifyIntegrity_head_errors(self) -> None:
        column_scheme = ColumnScheme(head=self.sample_head_scheme, measurements=self.sample_measurements_scheme)
        trial_site = TrialSite(df=pd.DataFrame(columns=['Bestandeseinheittypo', 'Baumart', 'Baumnummer']), metadata={})
    ...
    def test_verifyIntegrity_measurements_errors(self) -> None:
        column_scheme = ColumnScheme(head=self.sample_head_scheme, measurements=self.sample_measurements_scheme)
        trial_site = TrialSite(df=pd.DataFrame(
            columns=['Dtypo_1980', 'Aus_1980', 'D_2000', 'Aus_2000', 'D_2020', 'Aus_2020']), metadata={})
        self.assertRaises(ValueError, trial_site.verify_column_integrity, column_scheme)
    ...
    def test_expandColumnLabels(self):
        self.assertEqual(
            list(TrialSite.expand_column_labels(['Baumart', 'Baumnummer', 'D_2000', 'Aus_2000'])),
            [(-1, 'Baumart'), (-1, 'Baumnummer'), (2000, 'D'), (2000, 'Aus')]
        )

    def test_compressColumnLabels(self):
        self.assertEqual(
            list(TrialSite.compress_column_labels([(-1, 'Baumart'), (-1, 'Baumnummer'), (2000, 'D'), (2000, 'Aus')])),
