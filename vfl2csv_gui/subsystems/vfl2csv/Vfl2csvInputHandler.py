...
from vfl2csv_gui.subsystems.vfl2csv.Vfl2csvConversionWorker import (
    Vfl2csvConversionWorker,
)
...
class Vfl2csvInputHandler(AbstractInputHandler):
    ...
    def convert(
            self, output_dir: Path, settings: dict[str, bool] = None
    ) -> tuple[int, CommunicationSignals]:
        setting_create_form = settings["setting_create_form"]
        worker = Vfl2csvConversionWorker(
            output_dir,
            self.input_files,
            self.input_data,
            create_form=setting_create_form,
        )
    ...
    def sort(self):
        self.input_data = sorted(
            self.input_data, key=lambda input_data: input_data.string_representation()
        )
