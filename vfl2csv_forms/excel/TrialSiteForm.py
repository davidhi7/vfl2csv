...
class TrialSiteForm:
    def __init__(
            ...
            output_path: Path,
            formulae_columns: Iterable[FormulaColumn],
    ):
        self.df = trial_site.df
        self.metadata = trial_site.metadata
        self.output_path = output_path
        self.formulae_columns = formulae_columns
        self.sheet_name = trial_site.replace_metadata_keys(
            config["Output"]["excel_sheet_pattern"]
        )
        self.table_head_row = 5
        self.row_span = list(
            range(self.table_head_row, self.table_head_row + len(self.df.index) + 1)
        )
        self.worksheet = None
    ...
    def add_comment_column(self) -> None:
        ...
        cell_range = (
            f"A{self.row_span[1] + 1}:{break_column_name}{self.row_span[-1] + 1}"
        )
    ...
    def write_metadata(self) -> None:
        ...
        metadata_key_cells = zero_based_cell_range(
            ws, metadata_columns[0], 0, metadata_columns[0], 3
        ...
        metadata_value_cells = zero_based_cell_range(
            ws, metadata_columns[1], 0, metadata_columns[1], 3
        )
        ...
        for row in range(3):
            zero_based_cell(
                ws, metadata_columns[3], row
            ).style = styles.metadata_values_underlined.name
            zero_based_cell(
                ws, metadata_columns[3] + 1, row
            ).style = styles.metadata_values_underlined.name
            zero_based_cell(
                ws, metadata_columns[3] + 2, row
            ).style = styles.metadata_values_underlined.name
    ...
    def write_formulae_columns(self) -> None:
        for formulae_column in self.formulae_columns:
            if formulae_column.yielded_column is not None:
                ...
                continue
            self.first_empty_column += formulae_column.insert(
                self.first_empty_column, self.row_span, self.worksheet
            )
    ...
    def apply_table_formatting(self) -> None:
        ...
        for column_index in range(self.first_empty_column):
            ...
            if column_index < len(self.df.columns):
                zero_based_cell(
                    self.worksheet, column_index, self.table_head_row
                ).style = styles.table_head_border.name
            else:
                zero_based_cell(
                    self.worksheet, column_index, self.table_head_row
                ).style = styles.table_head.name
            ...
            if column_index < len(self.df.columns):
                ...
                style = dtypes_styles_mapping.get(
                    self.df.dtypes[column_index].name, styles.table_body_text
                ).name
                ...
                for row in self.row_span[1:]:
                    zero_based_cell(
                        self.worksheet, column_index, row
    ...
    def adjust_column_width(self) -> None:
        for column in range(self.first_empty_column):
            column_content: list[list[Cell]] = zero_based_cell_range(
                self.worksheet, column, self.row_span[0], column, self.row_span[-1]
            )
            if column_content[0][0].value is None:
                continue
            else:
                max_length = get_character_count(
                    column_content[0][0].value, decimal_digits=1
                )
            ...
            if len(column_content) > 1:
                ...
                if not (
                        isinstance(first_content_value, str)
                        and first_content_value.startswith("=")
                ):
                    ...
                    for row in column_content:
                        max_length = max(
                            max_length,
                            get_character_count(row[0].value, decimal_digits=1),
                        )
